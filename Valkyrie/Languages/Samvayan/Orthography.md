#conlang #samvay #valkyrie-world  #samvayan #worldbuilding 
# Romanization
## Consonants
Here, nasalized approximants are actually given their own row, even though they are evolved from, and often still considered, sequences of consonants. They may optionally be represented with a tilde over their second letter.

|             | -   | Labial | Dental | Cerebral | Palatal | Velar |
| ----------- | --- | ------ | ------ | -------- | ------- | ----- |
| Plosive     | -   | p b    | t d    | ṭ ḍ      | c j     | k g   |
| Fricative   | -   | f      | s      | z        | ś       | h     |
| Nasal       | -   | m      | n      | ṇ        | ń       | ŋ     |
| Approximant | -   | v      | l      | r        | y       | w     |
| Ãpprõxĩmãnt | -   | mv     | ns     | ṇz       | ńy      | ŋw    |
## Vowels
The approximant vowels are in the same order as the approximant consonants. Syllabic nasals are written as if they were a sequence of the syllabic approximant then n. The letter ʌ can be removed in most circumstances; it is only important for transliteration.

|             | -   | Back close | Front mid | Back mid | Front close | Open |
| ----------- | --- | ---------- | --------- | -------- | ----------- | ---- |
| True        | -   | u          | e         | o        | i           | a    |
| Approximant | -   | ʌv         | ʌl        | ʌr       | ^           | ʌw   |
