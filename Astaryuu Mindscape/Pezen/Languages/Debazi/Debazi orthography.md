#conlang #debaz #spectradom #debazi #worldbuilding 
# Romanization
## Consonants

|                 |              | Labial  | Alveolar   | Velar   | Guttural |
| --------------- | :----------: | ------- | ---------- | ------- | -------- |
| **Plosive**     |  **plain**   | p       | t          | k       | q        |
| ^               |  **voiced**  | b       | d          | g       |          |
| ^               |  **light**   | py/ps   | c          | ky/kh/ḳ |          |
| ^               | **l.voiced** | by/bz   | dy/dz      | gy/j    |          |
| ^               |   **dark**   | pʼ      | tʼ         | kʼ      | ʼ        |
| **Fricative**   |  **plain**   | f       | z s        | h       |          |
| ^               |  **light**   | fy/v/f̣ | ty sy/sh   | hy/ch/ċ |          |
| ^               |   **dark**   | fw/ff   | tw sw/ss/ṣ | ph/ḥ    | x        |
| **Approximant** |  **plain**   | yy/ẏ    | y          | w       | ‘        |
| ^               | **lateral**  |         | l          | lw/ll   |          |
| **Nasal**       |  **short**   | m       | n ń        | ng/ŋ    |          |
| ^               |     long     | mm      | nn nń      | n(ŋ)    |          |
| **Trill**       |  **plain**   |         | r          |         |          |
| ^               |  **light**   |         | zy/zh      |         |          |
| ^               |   **dark**   |         | r’         |         |          |
| **Implosive**   |   **dark**   | b’      | d’         |         |          |
For the non-sonorants in the above table that have two representations, use the second one in the coda. Using the second one in the onset is allowed, though. Astaryuu themself defaults to⟨ps bz kh⟩ intervocalically, but will use the ⟨y⟩ spelling if there is a consonant or a word boundary before them.
For the velar nasal, ŋ is in parentheses because it can be replaced by either of the representations for short /ŋ/, or by h.
For the sake of compatibility, digraphs are usually preferred over diacritics when it comes to consonants. However, as ⟨ċ ṣ ẏ ḥ ḳ ŋ⟩ have their own codepoints, they are more likely to be supported, especially ⟨ċ ŋ⟩ as they are in Latin Extended-A instead of Latin Extended Additional.
## Vowels
|               | Front     | <         | <        | Central   | <         | <        | Back      | <         | <        |
| ------------- | --------- | --------- | -------- | --------- | --------- | -------- | --------- | --------- | -------- |
|               | **light** | **plain** | **dark** | **light** | **plain** | **dark** | **light** | **plain** | **dark** |
| **Close**     | (ī)       | i         | (ï)      |           | <         | <        | (ÿ)       | u         | (ū)      |
| **Close-mid** | (î)       | ē/é/ee    | (ë)      | ^         | ^         | ^        | (ö)       | ō/ó/oo    | (û)      |
| **Open-mid**  | (ê)       | e         | (ȧ)      | (è)       | ȧ/à       | (ò)      | (ȧ)       | o         | (ô)      |
| **Open**      |           | <         | <        | (ä)       | a         | (å)      |           | <         | <        |
The accents and digraphs are listed for compatibility. A with overdot is in Latin Extended-B, as ȧ, so it should be acceptable in most of the same instances as ċ.
For vowels, the romanization does not distinguish light, dark, and plain vowels by default, as distinction is often unnecessary due to [[Debazi phonology]]. If needed, stick a y after the vowel for light vowels, a w after the vowel for dark vowels, and a ‘ before the vowel for plain vowels. These filler y, w, and ‘ are *not* actual consonants, and so do *not* have vowels filled in between them and following consonants if the conjugation calls for it. Light and dark vowels can also be replaced by diacritics as listed in the above table, but using a digraph is preferred here. Since the digraphs mess with clarity, it is recommended that dictionaries use the diacritics.
# Native script
The Debazi language natively uses a "logoabjad", like Ancient Egyptian or precolonial Mayan. This means that each symbol has a specific meaning, but can also be used for the first consonant in the root of the word. Sometimes, related words are used as non-pronounced semantic markers, particularly when two common words have the same consonant sequence.

When writing a word in Debazi, a character being used for its first consonant can also be modified, making it plain, light, or dark using diacritic-like marks. These are placed below the character, and are a small X-like mark, two horizontal ticks, and a small H-like mark, derived from the symbols for earth, wind, and fire respectively.

Debazi language authorities have made lists of "basic symbols", which are the most used symbols in Debazi and are commonly used both to build new graphemes and to write words non-logographically. The current standard is the "‘Amera-Macheda table", which contains 200 symbols, at least one for each of the consonants in Debazi. Overall, among these and other common symbols, about 1,000 symbols are commonly used in Debazi.

Due to the awkward state of Debazi vowels, some authors have begun writing vowels in addition to consonants, typically using simple radicals of words whose first vowel is the chosen one. This has yet to become standard, and is not used in informal writings, though the eight plain vowels often have agreed-upon symbols.